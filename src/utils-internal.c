...
int16_t dotProduct(int16_t * v1, int16_t * v2) {
  return (v1[0] * v2[0]) >> (2 * NUM_FRAC_BITS) + (v1[1] * v2[4]) >> (2 * NUM_FRAC_BITS) + (v1[2] * v2[8]) >> (2 * NUM_FRAC_BITS) + (v1[3] * v2[12]) >> (2 * NUM_FRAC_BITS);
}
...
int32_t renderQueueNumBytesFree() {
  ...
  if (sdStart != NULL && (uint8_t *) lastItem <= sdStart)
    ptr = sdStart;
  else if (audioStart != NULL && (uint8_t *) lastItem <= audioStart)
    ptr = audioStart;
  else if (controllerStart != NULL && (uint8_t *) lastItem <= controllerStart)
    ptr = controllerStart;
