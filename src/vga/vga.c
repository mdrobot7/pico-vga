...
static void initDMA() {
  ...
  dma_channel_configure(frameCtrlDMA, &frame_ctrl_config, &dma_hw->ch[frameDataDMA].al3_read_addr_trig, frameReadAddrStart, 1, false);
...
static irq_handler_t updateFramePtr() {
  ...
  // re-rendering it.
  if (displayConfig->interpolatedRenderingMode && (((uint8_t *) dma_hw->ch[frameCtrlDMA].read_addr) - 1) < frameBufferStart && (((uint8_t *) dma_hw->ch[frameCtrlDMA].read_addr) - 1) >= (blankLineStart + frameWidth)) {
